...
describe('RemoteModel', () => {
    ...
    describe('CRUD', () => {
        ...
        it('saves the model which does not exist', async () => {
            ...
            expect(result).to.be.instanceOf(Child);
            expect(result).to.be.instanceOf(RemoteModel);
            expect(result).to.be.equal(instance);
            expect(result).to.have.all.keys(['id', 'created_time', 'updated_time', 'active', '_changed', '_data', '_loaded']);
            expect(result.id).to.be.equal(1);
            expect(result.created_time).to.be.not.null;
            expect(result.updated_time).to.be.null;
        ...
        it('updates the model with defined id without load', async () => {
            ...
            expect(result.id).to.be.equal(1);
            expect(result.created_time).to.be.not.null;
            expect(result.updated_time).to.be.not.null;
            expect(result.active).to.be.true;
        });
